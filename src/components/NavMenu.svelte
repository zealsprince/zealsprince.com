...
>
  <button class="nav-toggle {open ? 'open' : ''}" on:click={() => open = !open} aria-label="Open navigation">
  ...
  {#if open}
    <div class="nav-list open" transition:fly={{ y: -10, duration: 200 }}>
      {#each items as item}
        <a class="nav-link" href={item.slug === 'index' ? '/' : `/${item.slug}`} on:click|preventDefault={() => navTo(item.slug)}>{item.navigation ?? item.title}</a>
      {/each}
...
.icon-lines line {
  ...
  transition: transform 0.3s ease-in-out, opacity 0.3s ease-in-out, stroke-width 0.3s;
}
